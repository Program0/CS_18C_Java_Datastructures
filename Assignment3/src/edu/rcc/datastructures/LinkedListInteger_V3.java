 ...
 * Only difference from LinkedListInt_V2 is this uses the Integer wrapper class
 */

package edu.rcc.datastructures;

public class LinkedListInteger_V3 {
	private Node head;
	private Node tail;
	Integer numberOfEntries;

	// Default constructor
	public LinkedListInteger_V3() {
		tail = null;
		head = null;
		numberOfEntries = 0;
	}
	 ...
	 */
	public boolean delete(Integer entry) {
		...
		// If not then go through the rest of the nodes
			...
			while (!deleted && tail != null) {
				if (tail.getData() == entry) {
					deleted = true;
					// Change the link set the previousNode's internal reference
